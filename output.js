//Sun Mar 16 2025 19:03:03 GMT+0000 (Coordinated Universal Time)
...
class _0x3f433d extends _0x280825 {
  ...
  async firstRequest() {
    try {
      const {
        stdout,
        stderr
      } = await execAsync('python Ruishu.py');
      if (stderr) {
    ...
    try {
      ...
      if (_0x43d9c9 == 0) {
        ...
        if (_0x402f9a.includes("您无此权限")) {
          ...
          return this.polymerize(_0x2beade, _0x3610fd);
        }
    ...
    try {
      ...
      if (_0x117b58 == 0) {
        ...
        if (_0x14117b.includes("您无此权限")) {
          ...
          await this.get_sign();
          this.log("重新获取sign成功，继续完成任务...");
          return this.food(_0x7cbaa1, _0x4b0ab1);
        }
  ...
  async userTask() {
    ...
    const totalAccounts = _0x49dfef.userList.length;
    const completedAccounts = _0x49dfef.userList.filter(user => user.taskCompleted).length + 1;
    const progressPercentage = completedAccounts / totalAccounts * 100;
    const progressBarLength = 20;
    const filledLength = Math.round(progressPercentage / 100 * progressBarLength);
    const progressBar = '[' + '*'.repeat(filledLength) + ' '.repeat(progressBarLength - filledLength) + ']';
    console.log("======= 等待所有账号完成任务，账号[" + this.name + "]已完成  =======  总进度" + progressBar + " " + progressPercentage.toFixed(0) + "%");
...
!(async () => {
  ...
  _0xa0ff1b();
  await Promise.all(_0x49dfef.userList.map(async _0x28b102 => {
    await _0x28b102.userTask();
  }));
  _0x49dfef.userList.sort((a, b) => a.index - b.index);
  for (let _0x28b102 of _0x49dfef.userList) {
    console.log("\n======= 账号[" + _0x28b102.index + "][" + _0x28b102.name + "] 日志 =======");
    _0x28b102.logs.forEach(log => console.log(log));
  }
  await _0x49dfef.showmsg();
  let _0x4210ea = Date.now(),
    _0x52abd1 = (_0x4210ea - _0x49dfef.startTime) / 1000;
  _0x49dfef.log("");
  const _0x4bb8d6 = {
    time: true
  };
  _0x49dfef.log("[" + _0x49dfef.name + "]运行结束，共运行了" + _0x52abd1 + "秒", _0x4bb8d6);
  process.exit(0);
